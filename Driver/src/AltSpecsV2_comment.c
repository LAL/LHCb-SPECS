 ...
 * On accéde Ã  la carte via le fichier de peripherique /dev/SpecsV2[0-3]
 * 
 * Le driver accédera au péripérique via une structure  
 * file_operations" qui permet certaines actions dont la pricipale ioctl: offre à
 * l'utilisateur la possibilite d'envoyer des commandes spÃ©cifiques.
 ...
 *
 */
struct sgdma_register {
  u32 status;
  u32 status_pad[3];
  u32 controlDMA;
  u32 controlDMA_pad[3];
  u32 next_descript;
  u32 nextdesc_pad[3];
...
/**
 * DMA Descriptor
 * See Table 23-9. DMA Descriptor Structure  of chapiter 23. Scatter-Gather DMA Controller Core
 * or Table 2-1: DMA Descriptor Field Description 
 * The SGDMA is not yet used SpecsV2
 **/

/**
 * Translation table PCI-to-Avalon Address Translation
 *
 */
...
#endif
{
  struct alt_dma_descriptor* desc;
  struct Aria5Specs_dev *specs_dev;
  ...
  specs_dev =dev_id;
  desc = (struct alt_dma_descriptor *) ((specs_dev->bar[BAR2]) + AVALON_DMA_CONTROLLER);
 ...
 * free the allocation of struct Aria5Specs_dev and the 16 struct Master
 * @param dev pointer of struct pci_dev
 */
...
/**
 * @details Management of all the functions of the Specs-v2 board
 * @param inode pointer of struct inode
 * @param file pointer of struct file
		    ...
		    unsigned long ioctl_param) 
{
  ...
  case IOCTL_BUS_IOP_READ :
    ...
    //else 
    //{
    // pBusIop.TransferSize=level_fifo; // n'est pas gèré au niveau user le read doit ramener des datas a revoir 
    //il faut que l'on arrive sur le read avec fifo_level ==  aux données demandées
    // rc = 1; 
    //}
  ...
  case IOCTL_BUS_IOP_WRITE :
    ...
    if (!copy_from_user(&pBusIop,( BUSIOPDATA*) ioctl_param, sizeof( BUSIOPDATA)))
       ...
       if (  (pBusIop.Address & MASK_FIFO_IN )  ==  ( FIFO_EMISSION_OUT & MASK_FIFO_IN) )
	 {
	   ...
	   for (i=0 ;i < 1000 ;i++)  // attente que l'on ai fini d'ecrire dans la fifo (PLX_IT2)
	     {
	       level_fifo = ioread32 (&master->fifoEmiOut_Status->fill_level);
	   ...
	   udelay(50); // il faut attendre encore un peu
	   if (level_fifo > 0) 
	     {
  ...
  case IOCTL_RESET :
	...
	iowrite32(0x0000, pReset);
	/*	if (ioread32 (specs_dev->bar[BAR2]+ RESET) != 0 )
	  {
...
/** @details function not used in user program
 * @param file pointer of struct file
...
/** @function not used by the user programm
 * @param file pointer of struct file
 * @param buf char pointer allocated by the user programm
 * @param count number of bytes to be copied in buf
 * @param pos offset in octets
